...
const promptSubmit = () => {
	...
	//todo: use switch on promptOptions.type?
	if (promptOptions.type === 'input' || promptOptions.type === 'textarea') {
		data = dataEl.value;
...
docReady(() => {
	...
	//todo: decide whether to split textarea out into its own 'if (...)'
	//there is a lot of shared stuff with type === input...
	if (promptOptions.type === 'input' || promptOptions.type === 'textarea') {
		dataEl = document.createElement(promptOptions.type);
		...
		dataEl.addEventListener('keyup', e => {
			...
			if (promptOptions.type !== 'textarea' && e.which === 13) {
				promptSubmit();
			}
			if (e.which === 27) {
				promptCancel();
